...
export const authApi = {
  ...
  getTwoFactorRecoveryCodes: async (): Promise<{ recovery_codes: string[] }> => {
    const response = await api.get("/user/two-factor-recovery-codes")
    return response.data
  },

  regenerateTwoFactorRecoveryCodes: async (): Promise<{ recovery_codes: string[] }> => {
  ...
  // メール認証完了
  verifyEmail: async (id: string, hash: string, signature: string): Promise<void> => {
  ...
  // メール認証コードの検証
  verifyEmailTwoFactorCode: async (data: EmailTwoFactorData): Promise<LoginResponse> => {
