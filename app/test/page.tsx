...
export default function TestPage() {
  ...
  const runBasicConnectionTest = async () => {
    try {
      ...
      addResult("基本接続テスト", "success", `${response.data.message}`);
    } catch (error: any) {
  ...
  const runRegistrationFlowTest = async () => {
    try {
      ...
      addResult(
        "登録フローテスト",
        "error",
        `テスト失敗: ${error.response?.data?.message || error.message}`
      );
    }
  };
    ...
    try {
      ...
      try {
        ...
        addResult(
          "ログインエラーテスト",
          "error",
          "存在しないユーザーでログインが成功しました（予期しない動作）"
        );
      } catch (error: any) {
        addResult(
          "ログインエラーテスト",
          "success",
          "存在しないユーザーのログインが正しく拒否されました"
        );
      }
    } catch (error: any) {
      addResult(
    ...
    try {
      ...
      try {
        ...
        addResult(
          "メール認証状態確認",
          "success",
          `認証状態: ${result.verified ? "認証済み" : "未認証"}`
        );
      } catch (error: any) {
        addResult(
          "メール認証状態確認",
          "error",
          `状態確認失敗: ${error.response?.data?.message || error.message}`
        );
      }
      ...
      try {
        ...
        addResult(
          "認証メール再送信",
          "error",
          `再送信失敗: ${error.response?.data?.message || error.message}`
        );
      }
    } catch (error: any) {
      ...
      try {
        ...
        try {
          ...
          addResult(
            "シークレットキー取得",
            "success",
            `シークレットキー: ${secretResponse.secret.substring(0, 8)}...`
          );
        } catch (error: any) {
          addResult(
            "シークレットキー取得",
    ...
    try {
      ...
      try {
        ...
        addResult(
          "メール認証コード送信",
          "error",
          `送信失敗: ${error.response?.data?.message || error.message}`
        );
      }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Laravel APIとの接続および認証フローの動作確認を行います
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <Button
                variant="outline"
                onClick={runRegistrationFlowTest}
                disabled={isRunning}
              >
                登録フロー
              </Button>
              ...
              <Button
                variant="outline"
                onClick={runEmailVerificationTest}
                disabled={isRunning}
              >
            ...
            <div className="space-y-4 text-sm">
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-slate-600">
                  <li>
                    POST /api/user/email-two-factor-authentication -
                    メール二段階認証有効化
                  </li>
                  <li>
                    DELETE /api/user/email-two-factor-authentication -
                    メール二段階認証無効化
                  </li>
                  <li>
                    POST /api/email-two-factor-code -
                    ログイン時メール認証コード送信
                  </li>
                  <li>
                    POST /api/email-two-factor-challenge - メール認証コード検証
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-slate-600">
                  <li>
                    ログインAPIで二段階認証が必要な場合、two_factor_method（"app"
                    | "email"）を返す
                  </li>
                  <li>
                    認証アプリの場合：既存の /api/two-factor-challenge を使用
                  </li>
                  <li>
                    メール認証の場合：/api/email-two-factor-challenge を使用
                  </li>
                </ul>
              </div>
